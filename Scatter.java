...
public class Scatter extends Graph{
    ...
    public Scatter(String analysis,JsonObject data,int startyr, int endyr) {
    ...
    public void createGraph() {
       //code for creating graph with data
        ...
        if(analysis == "Mortality") {
          ...
          vcontroller.scatterView(analysis, dataset);
          ViewController.createReport(data.toString());
            ...
            for(int i = 1; i <= data.size(); i++) {
               ...
               if(dataR != null) {
            	   for(int j = this.startyr; j<=this.endyr; j++) {
                       if(i == 1) {
                       Integer value = Integer.valueOf(j);
                       series1.add(new Year(value.intValue()), dataR.get(value.toString()).getAsDouble());
                       }
                   }
               }
               else {
           ...
           TimeSeries series2 = new TimeSeries("Expenditure(% of GDP)");
            ...
            vcontroller.scatterView2(analysis,dataset,dataset2);
            ViewController.createReport(data.toString());
        }
        else {
